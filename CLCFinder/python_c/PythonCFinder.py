...
class PythonCFinder():
    def __init__(self, project_path):
        ...
        self.import_python_files_c_test =[]
    ...
    def find_module_name(self):
        pattern1 = r'#include\s+<Python\.h>'
        pattern2 = r'\bPy_InitModule3\("([a-zA-Z0-9_]+)"'
        pattern3 = r'\bstruct\s+PyModuleDef\b'
                    ...
                    with (open(file_path, 'r', encoding='ISO-8859-1',errors='ignore') as f):
                        ...
                        for i, line in enumerate(lines):
                            ...
                            if re.search(pattern1, line):
                                self.import_python_files_c_test.append(file_path)
                                import_flag = True

                            # pattern2  module_name
                            match = re.search(pattern2, line)
                            if import_flag and match:
                                self.c_files.append(file_path)
                                name = match.group(1)
                                # print("- 1 -  Matched module name:", name)
                                self.module_name.append(name)

                            # pattern3
                            if import_flag and re.search(pattern3, line):
                                line_number = i + 1 + 2
                            if i + 1 == line_number:
                                match2 = re.search(r'"([a-zA-Z0-9_]+)"', line)
                                if match2:
                                    name = match2.group(1)
                                    # print("- 2 - Matched module name2:", name)
                                    self.module_name.append(name)
    ...
    def find_python_c(self, module_name):
    ...
    def find_main(self):
        ...
        for mn in self.module_name:
            self.find_python_c(mn)

        self.res_python = myutils.list_deduplicated(self.res_python)

        self.c_files = myutils.list_deduplicated(self.c_files)
...
if __name__ == '__main__':
    ...
    #     print('=================  pf.res_python  ===============================================')
    #     print(f'     pf.res_python的长度为{len(pcf.res_python)}')
    #     for r in pcf.res_python:
    #         print(r)
    #     for c in pcf.c_files:
    #         print(c)
    #     for x in pcf.import_python_files_c_test:
    #         print(x)
